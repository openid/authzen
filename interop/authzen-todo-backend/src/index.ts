...
import { checkJwt, authzMiddleware, authzBoxcarMiddleware } from "./auth";
...
Store.open().then((store) => {
  ...
  const checkBoxcarAuthz = authzBoxcarMiddleware(store);
  ...
  app.delete(
    "/todos",
    checkJwt,
    checkBoxcarAuthz("can_delete_todo"),
    server.deleteBoxcar.bind(server)
  );
