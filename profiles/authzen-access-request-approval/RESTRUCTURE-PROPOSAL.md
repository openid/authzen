...
Current repository note (2026-09-17): this is the historical relocation plan and source of the Appendix X verification scripts. A1 was committed as `9002d92`, A2 through `d73676b`, and A3 through `97a575f`. At the editor's request, their history is now consolidated in `arap-restructure`, tracking `origin/arap-restructure`; the former A1/A2/A3 branch names and the separate stacked pull-request plan below are superseded. The catalog split remains separate. See `A3-PLAN.md` for the subsequent editorial pass; historical baselines, line references, and acceptance criteria below are retained for audit.

Historical version 15 status: PR A1 commit 2 (verbatim relocation) was executed in the working tree, corrected after two independent reviews of the rendered result, re-verified, and approved by the editor on 2026-09-08; it was not yet committed at that checkpoint. PR A2 (wording) was the next pass. Pass 0 was signed off by the editor on 2026-09-07 after four review rounds; see `PASS0-REPORT.md`, `PASS0-INVENTORY.tsv`, `PASS0-ANCHORS.tsv`, `PASS0-EXPECTED-MOVES.tsv`, and `PASS0-SKELETON.md` in this directory.
